...
def backTracker(row: int, n: int, cols: Set[int], pos_diag: Set[int], neg_diag: Set[int], board: List[List[int]]) -> None:
    ...
    """
    if row == n:
        solved = [[r, c] for r in range(n) for c in range(n) if board[r][c] == 1]
        print(solved)
        return

    for column in range(n):
        pos_diagonal = row + column
        neg_diagonal = row - column
        
        if column in cols or pos_diagonal in pos_diag or neg_diagonal in neg_diag:
            continue
